...
import { type BuildOptions, bosContract, type PublishOptions } from "./contract";
...
import { syncAndGenerateSharedUi } from "./lib/shared-ui";
...
function getRegistryUrl(account: string, gatewayDomain: string): string {
  return buildRegistryConfigUrl(account, gatewayDomain);
}

async function fetchPublishedConfig(
  account: string,
  gatewayDomain: string,
): Promise<BosConfigType | null> {
  try {
    return await fetchBosConfigFromFastKv<BosConfigType>(`bos://${account}/${gatewayDomain}`);
  } catch {
    return null;
  }
}
...
async function buildEnvVars(
  ...
  env.BOS_ENABLE_SSR = config.ssr ? "true" : "false";
...
export default createPlugin({
  ...
  createRouter: (deps: BosDeps, builder) => ({
    dev: builder.dev.handler(async ({ input }) => {
      const { resolved, autoRemote } = await resolvePackages(["host", "ui", "api"], {
      ...
      // Keep shared.ui versions/policy synced and generate .bos artifacts.
      // Direction follows host mode:
      // - local host: catalog -> bos.config.json
      // - remote host: bos.config.json -> catalog
      try {
        ...
        if (sharedSync.catalogChanged) {
          // Ensure local deps match published host expectations.
          await run("bun", ["install"], { cwd: deps.configDir });
        }
        // Refresh loaded config for downstream runtimeConfig building.
        ...
        // Non-fatal: dev can still run with existing config.
      }
      ...
      if (appConfig.proxy) {
        ...
        if (!proxyUrl) {
          console.log();
          console.log(
            colors.error(`  ${icons.err} Proxy mode requested but no valid proxy URL found`),
          );
          console.log(
            colors.dim(`    Configure 'api.proxy' or 'api.production' in bos.config.json`),
          );
          console.log();
          return {
            status: "error" as const,
    ...
    start: builder.start.handler(async ({ input }) => {
      ...
      if (input.account && input.domain) {
        ...
        if (!remoteConfig) {
          console.error(`No config found for bos://${input.account}/${input.domain}`);
          return {
            status: "error" as const,
            url: "",
          };
    ...
    build: builder.build.handler(async ({ input }: { input: BuildOptions }) => {
      ...
      // Ensure shared deps are generated before builds.
      try {
        const sharedSync = await syncAndGenerateSharedUi({
          configDir,
          hostMode: "local",
        ...
        if (sharedSync.catalogChanged) {
          await run("bun", ["install"], { cwd: configDir });
        }
      } catch {
        // Non-fatal
      }
    ...
    publish: builder.publish.handler(async ({ input }: { input: PublishOptions }) => {
      ...
      // Ensure shared deps are synced from local catalog before publishing.
      try {
        const sharedSync = await syncAndGenerateSharedUi({
          configDir: deps.configDir,
          hostMode: "local",
      ...
      try {
        ...
        const registryContract = getRegistryNamespaceForAccount(account);
        const registryKey = getRegistryConfigKey(account, gatewayDomain);
        const registryUrl = getRegistryUrl(account, gatewayDomain);
    ...
    create: builder.create.handler(async ({ input }) => {
      ...
      // Fetch parent BOS config from the FastKV-backed registry
      async function fetchParentConfig(templateUrl: string) {
        return fetchBosConfigFromFastKv<BosConfigType>(templateUrl);
      }
      ...
      async function cloneTemplate(
        ...
        const baseTemplate = subdir ? `${owner}/${repo}/${subdir}` : `${owner}/${repo}`;
      ...
      async function verifyTemplate(gitTemplate: string): Promise<boolean> {
        try {
          ...
          await execa("git", ["ls-remote", `https://github.com/${owner}/${repo}.git`, "HEAD"], {
            timeout: 10000,
          });
          return true;
        } catch {
      ...
      if (usePrompts) {
        ...
        const { input: inquirerInput, confirm } = await import("@inquirer/prompts");
        ...
        if (!account) {
          account = await inquirerInput({
            ...
            validate: (val) => val.includes(".near") || "Must be a valid NEAR account",
          });
        }
        ...
        includeHost = await confirm({
          message: "Include host package locally? (No = use remote from everything.dev)",
          default: false,
        });

        includeGateway = await confirm({
          message: "Include gateway package locally? (No = use remote from everything.dev)",
          default: false,
        });
      }
      ...
      try {
        ...
        const gitTemplate = parentConfig.template || "near-everything/every-plugin/demo";
        ...
        await writeFile(join(destDir, "bos.config.json"), JSON.stringify(bosConfig, null, 2));
        ...
        const registerEffect = Effect.gen(function* () {
          ...
          if (!gatewayNovaAccount) {
            return yield* Effect.fail(
              new Error("gateway.nova.account is required for secrets registration"),
            );
          }
    ...
    depsUpdate: builder.depsUpdate.handler(async ({ input }) => {
      ...
      try {
        ...
        for (const [name, config] of Object.entries(sharedDeps)) {
          const version =
            (config as { version?: string; requiredVersion?: string }).version ||
        ...
        for (const [name, newVersion] of Object.entries(updatedPkg.dependencies)) {
          const existing = sharedDeps[name] as
            | { version?: string; requiredVersion?: string }
            | undefined;
          const oldVersion =
            existing?.version ??
            (existing?.requiredVersion ? existing.requiredVersion.replace(/^[^\d]*/, "") : "");
    ...
    update: builder.update.handler(async ({ input }) => {
      ...
      const registryUrl = getRegistryUrl(account, gateway);
      ...
      try {
        const remoteConfig = await fetchPublishedConfig(account, gateway);
        ...
        if (!remoteConfig) {
          return {
            ...
            catalogUpdated: false,
            packagesUpdated: [],
            error: `No config found for bos://${account}/${gateway}. Run 'bos publish' first.`,
          };
        }
        ...
        // Sync shared deps from published config into local catalog and
        // generate .bos artifacts.
        const sharedSync = await syncAndGenerateSharedUi({
          configDir,
          hostMode: "remote",
        });
        ...
        // Ensure dependencies are actually installed after syncing catalog.
        await run("bun", ["install"], { cwd: configDir });
        ...
        return {
          ...
          registryUrl,
          hostUrl,
          catalogUpdated: sharedSync.catalogChanged,
          packagesUpdated,
          filesSynced,
        };
      } catch (error) {
    ...
    dockerRun: builder.dockerRun.handler(async ({ input }) => {
      ...
      const dockerEffect = Effect.gen(function* () {
        ...
        const containerId =
          input.detach && result.stdout ? result.stdout.trim().slice(0, 12) : "attached";
