...
function getHelpHeader(config: BosConfig | null, configPath: string): string {
  ...
  lines.push(`  ${icons.config} ${gradients.cyber("everything-dev")} ${colors.dim(`v${version}`)}`);
...
async function main() {
  ...
  program.name("bos").version("1.0.0").addHelpText("before", getHelpHeader(config, configPath));
  ...
  program
    ...
    .action(async () => {
      ...
      for (const remoteName of result.remotes) {
        ...
        console.log(
          color(`  ┌─ ${remoteName.toUpperCase()} ${"─".repeat(46 - remoteName.length)}┐`),
        );
        console.log(
  ...
  program
    ...
    .option("--ssr", "Enable local UI SSR")
  ...
  program
    ...
    .option("--account <account>", "NEAR account to fetch config from the FastKV registry")
  ...
  program
    .command("publish")
    .description("Build, deploy, and publish to the FastKV registry (full release)")
    ...
    .option("--path <path>", "Registry path override", "bos.config.json")
...
Release Workflow:
  ...
  2. Deploy to Zephyr Cloud (updates production URLs)
				3. Publish config to the FastKV registry
...
`,
    ...
    .action(async (pkgs: string, options) => {
      ...
      if (!options.dryRun) {
        ...
        console.log(
          colors.green(`  ${icons.ok} Built & deployed: ${buildResult.built.join(", ")}`),
        );
        console.log();
      }

      console.log(
        `  ${icons.pkg} ${options.dryRun ? "Dry run:" : "Step 2/3:"} Publishing to the FastKV registry...`,
      );
      ...
      if (result.status === "error") {
        console.error(
          colors.error(`${icons.err} Publish failed: ${result.error || "Unknown error"}`),
        );
        process.exit(1);
      }
      ...
      console.log(colors.green(`  ${icons.ok} Published to the FastKV registry`));
  ...
  const create = program.command("create").description("Scaffold new projects and remotes");

  create
    .command("project")
    .description("Create a new BOS project")
    .argument("<name>", "Project name")
    .option("-a, --account <account>", "NEAR mainnet account (e.g., myname.near)")
    .option("--testnet <account>", "NEAR testnet account (optional)")
    .option("-t, --template <url>", "Template BOS URL (default: bos://every.near/everything.dev)")
  ...
  const gateway = program.command("gateway").description("Manage gateway deployment");
  ...
  program
    ...
    .action(async (name: string, options: { network: string }) => {
      ...
      if (result.status === "error") {
        console.error(
          colors.error(`${icons.err} Registration failed: ${result.error || "Unknown error"}`),
        );
        process.exit(1);
      }
  ...
  const secrets = program.command("secrets").description("Manage encrypted secrets via NOVA");
  ...
  program
    ...
    .action(
      async (options: {
        ...
        console.log(`  ${colors.dim("URL:")}      ${colors.cyan(result.registryUrl)}`);
        ...
        if (result.packagesUpdated.length > 0) {
          console.log(
            colors.green(`  ${icons.ok} Updated packages: ${result.packagesUpdated.join(", ")}`),
          );
        }
  ...
  const depsCmd = program.command("deps").description("Manage shared dependencies");
  ...
  program
    ...
    .action(async () => {
      ...
      try {
        ...
        if (result.status === "error") {
          console.error(
            colors.error(`${icons.err} Login failed: ${result.error || "Unknown error"}`),
          );
          process.exit(1);
        }
  ...
  program
    ...
    .action(async () => {
      ...
      if (result.status === "error") {
        console.error(
          colors.error(`${icons.err} Logout failed: ${result.error || "Unknown error"}`),
        );
        process.exit(1);
      }
      ...
      if (result.summary) {
        ...
        if (result.status === "leaks_detected") {
          ...
          if (result.summary.orphanedProcesses > 0) {
            console.log(
              colors.error(`     - ${result.summary.orphanedProcesses} orphaned process(es)`),
            );
          }
