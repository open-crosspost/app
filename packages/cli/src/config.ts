...
export async function resolvePackages(
  ...
  for (const pkg of packages) {
    ...
    if (config && pkg in config.app) {
      ...
      if (pkg === "host") {
        port = isLocalDevelopmentTarget(pkgConfig.development)
          ? DEFAULT_HOST_PORT
          : parsePort(pkgConfig.development);
        url =
          mode === "remote"
            ? pkgConfig.production
            : resolveHostDevelopmentUrl(pkgConfig.development);
      } else {
...
function isLocalDevelopmentTarget(value: string | undefined): boolean {
  return typeof value === "string" && value.startsWith(LOCAL_PREFIX);
}

function resolveHostDevelopmentUrl(value: string | undefined): string {
...
export function getHostDevelopmentPort(value: string | undefined): number {
  return parsePort(resolveHostDevelopmentUrl(value));
}
...
function buildRuntimeConfig(config: BosConfig, env: "development" | "production"): RuntimeConfig {
  ...
  return {
    ...
    hostUrl:
      env === "development"
        ? resolveHostDevelopmentUrl(config.app.host.development)
