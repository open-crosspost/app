...
export const runLoginFlow = (
	...
	Effect.gen(function* () {
		...
		yield* Effect.asVoid(
			recorder.recordEvent("pageload", "/login", {
				url: `${opts.baseUrl}/login`,
			}),
		);
		...
		yield* Effect.tryPromise({
			try: () =>
				page.waitForSelector(connectButtonSelector, { timeout: opts.timeout }),
			catch: () =>
		...
		yield* Effect.asVoid(
			recorder.recordEvent("auth_start", "wallet_connect_initiated"),
		);

		if (opts.headless && opts.stubWallet) {
			yield* Effect.logInfo("Stubbing wallet connection in headless mode");
			yield* Effect.asVoid(
				recorder.recordEvent("custom", "wallet_stubbed", {
			...
			const hasSignInButton = yield* Effect.tryPromise({
				try: async () => {
					try {
						await page.waitForSelector('button:has-text("sign in as")', {
							timeout: 5000,
						});
						return true;
					} catch {
			...
			if (hasSignInButton) {
				yield* Effect.asVoid(
					recorder.recordEvent("custom", "sign_in_button_appeared"),
				);
			} else {
				yield* Effect.asVoid(
			...
			yield* Effect.asVoid(
				recorder.recordEvent("auth_complete", "login_flow_completed_stubbed"),
			);
		} else {
			...
			yield* clickElement(page, connectButtonSelector);
			yield* Effect.asVoid(
				recorder.recordEvent("click", "connect_wallet_button"),
			);
			...
			yield* Effect.logInfo(
				"NEAR wallet popup opened - waiting for user interaction",
			);
			yield* Effect.asVoid(
				recorder.recordEvent("custom", "awaiting_user_approval"),
			);
			...
			if (popupClosed) {
				yield* Effect.asVoid(
					recorder.recordEvent("popup_close", "near_wallet_popup"),
				);
			}
			...
			if (hasSignIn) {
				yield* Effect.asVoid(
					recorder.recordEvent("custom", "wallet_connected"),
				);
			...
			yield* Effect.asVoid(
				recorder.recordEvent("auth_complete", "login_flow_completed"),
			);
		}
...
export const runNavigationFlow = (
	...
	Effect.gen(function* () {
		yield* Effect.logInfo(
			`Running navigation flow through ${routes.length} routes`,
		);
		...
		for (const route of routes) {
			...
			yield* Effect.asVoid(
				recorder.recordEvent("interval", `visited_${route}`),
			);
		}
...
export const runClickFlow = (
	...
	Effect.gen(function* () {
		yield* Effect.logInfo(
			`Running click flow with ${selectors.length} interactions`,
		);
		...
		for (const { selector, label } of selectors) {
			...
			if (exists) {
				...
				yield* Effect.asVoid(
					recorder.recordEvent("click", label, { selector }),
				);
				yield* sleep(500);
			} else {
