...
export const getPortsToMonitor = (config?: MonitorConfig): Effect.Effect<number[]> =>
  Effect.gen(function* () {
    if (config?.ports && config.ports.length > 0) {
      yield* Effect.logDebug(`Using configured ports: ${config.ports.join(", ")}`);
      return config.ports;
    }
    ...
    if (result?.runtime) {
      const ports = [result.runtime.hostUrl, result.runtime.ui.url, result.runtime.api.url]
        .map((url) => {
          try {
            return parseInt(new URL(url).port || "0", 10);
          } catch {
            return 0;
          }
        })
...
export const getConfigPathSafe = (config?: MonitorConfig): Effect.Effect<string | null> =>
  Effect.tryPromise({
    try: async () => {
...
export const createSnapshot = (
  ...
  Effect.gen(function* () {
    ...
    const boundPorts = Object.values(portInfo).filter((p) => p.state !== "FREE");

    yield* Effect.logInfo(`Found ${boundPorts.length} bound ports`);

    const rootPids = boundPorts.map((p) => p.pid).filter((pid): pid is number => pid !== null);

    const processes = rootPids.length > 0 ? yield* platform.getProcessTree(rootPids) : [];
    ...
    yield* Effect.logDebug(`Total process RSS: ${(totalRss / 1024 / 1024).toFixed(1)}MB`);
    ...
    yield* Effect.logInfo(`Snapshot created at ${new Date(snapshot.timestamp).toISOString()}`);

    return snapshot;
  });

export const createSnapshotWithPlatform = (config?: MonitorConfig): Effect.Effect<Snapshot> =>
  withPlatform(createSnapshot(config));

export const findProcessesByPattern = (
  patterns: string[],
): Effect.Effect<ProcessInfo[], never, PlatformService> =>
  Effect.gen(function* () {
    yield* Effect.logDebug(`Finding processes matching: ${patterns.join(", ")}`);

    const platform = yield* PlatformService;
    const allProcesses = yield* platform.getAllProcesses();

    const matched = allProcesses.filter((proc) =>
      patterns.some((pattern) => proc.command.toLowerCase().includes(pattern.toLowerCase())),
    );

    yield* Effect.logInfo(`Found ${matched.length} processes matching patterns`);

    return matched;
  });

export const findBosProcesses = (): Effect.Effect<ProcessInfo[], never, PlatformService> => {
...
export const waitForProcessDeath = (pid: number, timeoutMs = 5000): Effect.Effect<boolean> =>
  Effect.gen(function* () {
    yield* Effect.logDebug(`Waiting for PID ${pid} to die (timeout: ${timeoutMs}ms)`);
...
export const waitForPortFree = (
  ...
  Effect.gen(function* () {
    yield* Effect.logDebug(`Waiting for port :${port} to be free (timeout: ${timeoutMs}ms)`);
    const platform = yield* PlatformService;
    const start = Date.now();
    ...
    if (!isFree) {
      yield* Effect.logWarning(`Port :${port} still bound after ${timeoutMs}ms`);
    }
