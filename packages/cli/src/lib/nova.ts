...
export const getNovaConfig = Effect.gen(function* () {
  ...
  if (!accountId || !sessionToken) {
    return yield* Effect.fail(
      new Error("NOVA credentials not configured. Run 'bos login' to authenticate with NOVA."),
    );
  }
...
export const registerSecretsGroup = (nova: NovaSdk, nearAccount: string, novaAccount: string) =>
  Effect.gen(function* () {
...
export const uploadSecrets = (nova: NovaSdk, groupId: string, secrets: Record<string, string>) =>
  Effect.gen(function* () {
