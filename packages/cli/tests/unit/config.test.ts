...
describe("Config Loading System", () => {
  ...
  describe("Local Config Loading", () => {
    it("loads simple config from disk", async () => {
      ...
      await writeFile(join(tempDir, "bos.config.json"), JSON.stringify(config, null, 2));
    ...
    it("validates config against schema", async () => {
      ...
      await writeFile(join(tempDir, "bos.config.json"), JSON.stringify(invalidConfig, null, 2));
  ...
  describe("Config with Extends (Local)", () => {
    it("loads config that extends another local config", async () => {
      ...
      await writeFile(join(parentDir, "bos.config.json"), JSON.stringify(parentConfig, null, 2));
    ...
    it("detects circular extends and throws ConfigCircularExtendsError", async () => {
      ...
      await expect(loadConfig({ cwd: dirA })).rejects.toThrow(ConfigCircularExtendsError);
    });
  });
  ...
  describe("Package Resolution", () => {
    ...
    it("resolves local host targets to the default localhost URL", async () => {
      const config: BosConfig = {
        account: "test.near",
        gateway: {
          development: "http://localhost:8787",
      ...
      await loadConfig({ cwd: tempDir });
      const result = await resolvePackages(["host"], {
        host: "local",
      });

      expect(result.resolved.host.mode).toBe("local");
      expect(result.resolved.host.port).toBe(3000);
      expect(result.resolved.host.url).toBe("http://localhost:3000");
    });
  });
  ...
  describe("Runtime Config Generation", () => {
    ...
    it("maps local host targets to a localhost runtime URL", async () => {
      const config: BosConfig = {
        account: "test.near",
        gateway: {
          development: "http://localhost:8787",
    ...
    it("computes federation entry URLs", async () => {
      ...
      const result = await loadConfig({ cwd: tempDir, env: "development" });
      expect(result?.runtime.ui.entry).toBe("http://localhost:3002/remoteEntry.js");
      expect(result?.runtime.api.entry).toBe("http://localhost:3014/remoteEntry.js");
    });
  });
