...
describe("Config Loading System", () => {
  ...
  describe("Config with Extends (BOS URL)", () => {
    ...
    it("times out after 10 seconds on slow BOS URL", async () => {
      // Timeout behavior is covered in the FastKV client helper.
    });

    it("throws ConfigFetchError on network failure", async () => {
      delete process.env.REGISTRY_FASTKV_MAINNET_URL;

      const childDir = join(tempDir, "bos-url-error");
      await mkdir(childDir, { recursive: true });
      await writeFile(
        join(childDir, "bos.config.json"),
        JSON.stringify({
      ...
      await expect(loadConfig({ cwd: childDir, force: true })).rejects.toBeInstanceOf(
        ConfigFetchError,
      );
    });
  });
  ...
  describe("Runtime Config Generation", () => {
    ...
    it("computes federation entry URLs", async () => {
      ...
      const result = await loadConfig({ cwd: tempDir, env: "development" });
      expect(result?.runtime.ui.entry).toBe("http://localhost:3002/mf-manifest.json");
      expect(result?.runtime.api.entry).toBe("http://localhost:3014/mf-manifest.json");
    });
  });
