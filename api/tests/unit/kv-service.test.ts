...
describe("KvService", () => {
  ...
  describe("setValue", () => {
    it("creates a new key-value pair", async () => {
      ...
      const result = await Effect.runPromise(service.setValue("my-key", "my-value", testOwner));
    ...
    it("updates an existing key owned by same user", async () => {
      ...
      const result = await Effect.runPromise(service.setValue("update-key", "updated", testOwner));
  ...
  describe("getValue", () => {
    ...
    it("throws NOT_FOUND for non-existent key", async () => {
      ...
      const exit = await Effect.runPromiseExit(service.getValue("missing-key", testOwner));
    ...
    it("throws FORBIDDEN when accessing key owned by another user", async () => {
      ...
      const exit = await Effect.runPromiseExit(service.getValue("private-key", otherOwner));
