...
export const Route = createFileRoute("/_layout/_authenticated/results/")({
  component: ResultsPage,
});

function ResultsPage() {
  const navigate = useNavigate();
  const { summary, results, errors, request, clearSubmissionOutcome } = useSubmissionResultsStore();

  const { submitPost, isPosting } = useSubmitPost();

  const handleRetry = async (errorDetail: ErrorDetail) => {
    if (!request || !errorDetail.details?.platform || !errorDetail.details?.userId) {
    ...
    const accountToRetry = selectedAccounts.find(
      (acc: ConnectedAccount) =>
        acc.platform === platformToRetry && acc.profile?.userId === userIdToRetry,
    );

    if (!accountToRetry) {
      console.error("Could not find account to retry in the original request:", {
        platformToRetry,
        userIdToRetry,
        selectedAccounts,
      });
    ...
    return (
      <div className="w-full max-w-2xl mx-auto text-center py-10">
        ...
        <p className="text-gray-500 mb-6">
          It looks like you haven't submitted any posts yet, or the results have been cleared.
        </p>
        <Button onClick={() => navigate({ to: "/editor" })}>Go to Editor</Button>
      </div>
    );
  }

  const getAccountForError = (error: ErrorDetail): ConnectedAccount | undefined => {
    if (request && error.details?.platform && error.details?.userId) {
      ...
      return request.selectedAccounts.find(
        (acc: ConnectedAccount) => acc.platform === platform && acc.profile?.userId === userId,
      );
    }
    return undefined;
  };

  const getAccountForSuccess = (success: SuccessDetail): ConnectedAccount | undefined => {
    if (request) {
                ...
                >
                  {account ? (
                    ...
                    <p className="font-medium text-green-700 dark:text-white">
                      Successfully posted to {success.platform} (User ID: {success.userId})
                    </p>
                  )}
                  {success.details && typeof success.details === "string" && (
                    <p className="text-sm text-green-600 dark:text-white mt-1">{success.details}</p>
                  )}
                  {success.details &&
                    ...
                    <div className="flex-grow mr-4">
                      {account ? (
                        ...
                        <p className="font-medium text-red-700">
                          Failed to post to{" "}
                          {(error.details?.platform as string) || "Unknown Platform"} (User ID:{" "}
                          {(error.details?.userId as string) || "Unknown User"})
                        </p>
                      )}
                      <div className="mt-2 text-red-700">
                        <p className="text-sm text-red-500 mt-1">{`${error.code}: `}</p>
